(function(basis) {
	var _safeAttributeSet = function(_e, attr, val) { 
        ...
        if (lattr.substring(0, 7) === 'observe') {
			_e.addEvent(attr.substring(7).toLowerCase(), val);
		} else {
	...
	var createDOM = function(n, node_args) {
		...
		if ((n === "a") && !attrs.href) {
			_e.setProperty("href", "#");
			_e.addEvent("click", function() { return false; });
		}
		...
		for (var i = nodeStart; i <  node_args.length; ++i) {
			if (typeof(node_args[i]) === 'function') {
				_e.inject(node_args[i]());
			} else if ( (typeof(node_args[i]) === 'string') || (typeof(node_args[i]) === 'number')) {
				_e.inject(document.createTextNode(node_args[i]));
			} else {
				_e.inject(node_args[i]);
			}
		}
